...
router.post("/register", async (req, res) => {
  ...
  if (user) return res.status(400).send("User alreay existing");
...
router.post("/login", async (req, res) => {
  ...
  if (!result) return res.status(400).send("check the username password");
  ...
  return res.status(200).header({ "x-auth-token": token }).send("login post");
});
...
// db.verificationSession.insert({data:"data3",endAt:new Date(Date.now()+60*2*1000)})
